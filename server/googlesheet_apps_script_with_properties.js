// Google Apps Script (doPost) - スクリプトプロパティ版
...
// 8. デプロイ後、ウェブアプリのURLをコピー
// 9. .envファイルのGAS_WEBHOOK_URLに設定

// スクリプトプロパティの設定方法:
// 1. Google Apps Scriptエディタで「プロジェクトの設定」（⚙️アイコン）をクリック
// 2. 「スクリプト プロパティ」タブを選択
// 3. 「プロパティを追加」をクリックして以下を設定:
//    - SHARED_SECRET: 任意のランダムな文字列（例: oekaki_battler_line_to_gas_secret_shiyow5）
//    - DRIVE_FOLDER_ID: Google DriveフォルダID（例: 1JT7QnTcSrLo2AC4p580V7fa_MScuVd4G）
//    - SPREADSHEET_ID: Google SpreadsheetsのスプレッドシートID（例: 1asfRGrWkPRszQl4IUDO20o9Z7cgnV1bEVKVNt6cmKfM）
//    - GOOGLE_API_KEY: Google Gemini APIキー（オプション - 現在は使用していません）
...
function doPost(e) {
  try {
    ...
    if (action === 'register_character_manual') {
      return handleRegisterCharacterManual(payload, SPREADSHEET_ID);
    }

    // AI自動生成でキャラクター登録
    if (action === 'register_character_auto') {
      return handleRegisterCharacterAuto(payload, SPREADSHEET_ID);
    }
...
// 手動入力でキャラクター登録
function handleRegisterCharacterManual(payload, spreadsheetId) {
  try {
...
function handleRegisterCharacterAuto(payload, spreadsheetId) {
  try {
    var imageUrl = payload.imageUrl;
    var fileId = payload.fileId;
